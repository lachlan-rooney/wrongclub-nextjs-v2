...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Always set user - try database first, fall back to auth data
      let userToSet: any = {
        id: data.user.id,
      ...
      try {
        ...
        console.log('Database fetch failed, using basic user info')
      }
  ...
  const handleAppleLogin = async () => {
    try {
      ...
      setError('An error occurred with Apple login.')
    }
  }
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4 mb-6">
          ...
          <div className="flex justify-end">
            <Link
              ...
              className="text-sm text-[#5f6651] hover:text-[#4a5040] font-medium"
            >
              Forgot password?
            </Link>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full px-6 py-3 bg-[#5f6651] text-white rounded-xl font-semibold hover:bg-[#4a5040] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="text-center">
          <p className="text-gray-600">
            ...
            <Link href="/signup" className="text-[#5f6651] hover:text-[#4a5040] font-semibold">
              Sign up
            </Link>
          </p>
        </div>
